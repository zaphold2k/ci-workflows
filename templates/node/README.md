...
6. Generate this repository's flow documentation block once:
   ...
   node path/to/ci-workflows/scripts/render-docs.mjs \
     --language node --branch-model simple --main-branch main \
...
Node-specific: needs a `package-lock.json` for dependency caching, and `lint`/`typecheck`/`build` npm scripts if you want those stages (they're skipped with `--if-present` when absent).
