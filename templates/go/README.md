...
6. Generate this repository's flow documentation block once:
   ...
   node path/to/ci-workflows/scripts/render-docs.mjs \
     --language go --branch-model simple --main-branch main \
...
Go-specific: needs a `go.sum`. The default test command emits both a coverage profile and JSON test output in one run; `golangci-lint` is installed automatically.
