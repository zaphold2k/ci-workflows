...
6. Generate this repository's flow documentation block once:
   ...
   node path/to/ci-workflows/scripts/render-docs.mjs \
     --language python --branch-model simple --main-branch main \
...
Python-specific: needs a `pyproject.toml` (the default install command is `pip install -e .[dev]`) and `pytest-cov` for the default test command's coverage output.
